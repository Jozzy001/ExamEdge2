...
const SubjectSelect = ({ onNavigate, onBack, mode, examType = "jamb", university = null, allowedSubjects = null }) => {
  ...
  const handleStartCBT = () => {
    ...
    onNavigate("cbt", null, null, selected, university)
  }
          ...
          <>
            <div className="ee-card" style={{ marginBottom: 16 }}>
              <p style={{ fontSize: 13, color: "var(--text2)", lineHeight: 1.6 }}>
                {examType === "jamb"
                  ? <>🎓 <strong style={{ color: "var(--text)" }}>JAMB format:</strong> English + 3 other subjects<br />Select up to <strong style={{ color: "var(--primary)" }}>4 subjects</strong>.</>
                  : <>🏛️ <strong style={{ color: "var(--text)" }}>Post-UTME:</strong> Select your subjects.</>
                }
                {selected.length > 0 && (
            ...
            {subjects.map((subject, index) => {
              ...
              return (
                <button key={index} onClick={() => !isDisabled && toggleSubject(subject)} style={{
